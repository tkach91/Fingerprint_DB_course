...
<script type="text/javascript" src="http://db.loc/assets/js/ext/plugins/searchfield.js"></script>
    ...
    <script type="text/javascript">
		...
		items:[{
			...
			handler: function() {
				var Employee = gridEmployees.getStore().recordType;
				var e = new Employee({
					emp_login: 'Новый логин',
					emp_first_name: 'Новое имя',
					emp_last_name: 'Новая фамилия',
					emp_post_id: '27',
					emp_divi_id: '4'
				});
				gridEmployees.stopEditing();
				strEmployees.insert(0, e);
				gridEmployees.startEditing(0, 0);
				}
			}, '-', {
					...
					Ext.Msg.show({
						...
						fn: function(btn) {
							if (btn == 'yes') {
								...
								Ext.Ajax.request({
									url: BASE_URL + 'employee/ext_delete',
									method: 'POST',
									params: { postdata: data }
								});
								strEmployees.load();
...
<body>
	...
	<select name='p_post' id='p_post' style="display: none;">>
	<? 
	...
	<select name='d_division' id='d_division' style="display: none;">>
	<? 
